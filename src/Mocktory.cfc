		...
		for (var key in mockDescriptor) {
			if (key != "$object" && key != "$class") {
				...
				mockDescriptor[key].each(function (descriptor) {
					...
					var isFunction = StructKeyExists(mockObject, key) && IsCustomFunction(mockObject[key]);
					var functionName = isFunction ? key : "get" & key;
					...
					if (arguments.descriptor.keyExists("$times")) {
						...
						// The between assertion expects the actual value as the first argument.
