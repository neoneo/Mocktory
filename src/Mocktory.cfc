...
component {
				...
				for (var descriptor in mockDescriptor[key]) {
					...
					for (var comparison in ["$times", "$atLeast", "$atMost", "$between"]) {
						if (descriptor.keyExists(comparison)) {
							var count = this.callCount(callLog, functionName, descriptor.$args ?: NullValue());
							if (comparison == "$times") {
	...
	private Numeric function callCount(required Struct callLog, required String methodName, required Any args) {
